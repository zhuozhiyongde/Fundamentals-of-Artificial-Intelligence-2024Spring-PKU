...
-   多层感知器（MLP）和卷积神经网络（CNN）都属于前馈神经网络（FNN），它们将一个数据样本作为输入，并输出一个结果，例如将图像输入并得到类标签。
...
词的表示（Word Representation）：将词转换为计算机可以理解的形式的过程。词的表示有两种主要方法：one-hot 编码和词嵌入（Word Embedding）。
...
-   用⼀组浮点数向量来表示⼀个单词，也即将单词映射到一个连续的向量空间
...
-   允许进行语义操作，如向量相加和相减 `King − Man + Women = Queen`

    这代表词嵌⼊中的特征包含了诸如 “性别” 和 “位置” 等信息。

如何学习词嵌入：现有的算法通过阅读⼤量⽂本⽂档来学习词嵌⼊表，以发现其中的模式，这是⼀种自监督（self-supervised） 学习⽅法。
...
-   Skip-gram：通过一个词预测它周围的词（上下文），最大化正确的上下文词的预测概率
-   CBOW：通过周围的词（上下文）预测一个中间的词，最大化正确的中间词的预测概率
...
$$
\begin{array}{l}{{h_{t}=tanh\left(W_{h}x_{t}+U_{h}h_{t-1}+b_{h}\right)}}\\{{y_{t}=\sigma_{y}(W_{y}h_{t}+b_{y})}}\end{array}
$$
...
-   $W_h$、$U_h$、$b_h$ 是隐状态的参数
-   $W_y$、$b_y$ 是输出的参数
-   $\sigma_y$ 是输出的激活函数，$\sigma_h$ 就是 $tanh$

局限性：隐状态的信息会随着时间步的增加而衰减，这意味着 RNN 只能处理短序列，长序列的信息会被遗忘。也即很难维持较为长期的信息。（考虑一个值连续应用多次 $tanh$ 会发生什么）
...
输入门分为两部分：一个输出向量，决定我们要把候选向量的哪些部分添加到细胞状态中；一个候选（信息）向量，决定我们要在细胞状态中存储那些新信息。
...
因而我们需要一个输出向量决定我们要在细胞状态中存储候选向量的那些部分。其通过如下公式计算：

$$
i_t = sigmoid([h_{t-1}, x_t] W_i + b_i)
$$
...
$$
\boldsymbol{C_t} = tanh([h_{t-1}, x_t] \boldsymbol{W_C} + b_C)
$$

其中：

-   $\boldsymbol{C_t}$: 当前时刻的信息向量（information vector）
-   $\boldsymbol{W_C},b_C$: 信息向量的权重矩阵、偏置项
-   $\boldsymbol{C_{t-1}}$: 前一时刻的细胞状态
...
$$
\boldsymbol{C_t} = \boldsymbol{f_t} \cdot \boldsymbol{C_{t-1}} + \boldsymbol{i_t} \cdot \boldsymbol{C_t}
$$
...
$$
h_t = o_t \odot tanh(\boldsymbol{C_t})
$$
...
> 不行，因为 ReLU 函数的输出范围是 $[0, +\infty)$，而门控函数的输出范围是 $[0, 1]$（更符合门控单元的定义），这样会导致信息流失。不过也有一些变种的 LSTM 使用了 ReLU 来替换 Sigmoid 函数，如 GRU。

当向向量输⼊信息时，为什么我们使用 tanh ⽽不是 Sigmoid？

> tanh 函数的输出范围是 $(-1, 1)$，而 Sigmoid 函数的输出范围是 $[0, 1]$，这样可以使得信息的变化范围更大，反向传播时更容易传递。
